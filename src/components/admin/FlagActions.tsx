...
export interface FlagInitialData {
  word: string
  slug: string
...
type ActionStatus = 'idle' | 'resolving' | 'dismissing' | 'resolved' | 'dismissed' | 'error'

// ─── Shared input style ───────────────────────────────────────────────────────

const inputClass = 'w-full px-3 bg-white/5 border border-white/8 rounded-lg text-sm text-foreground placeholder:text-muted-foreground/60 outline-none font-body transition-all duration-200 focus:border-[#c8f135]/30 focus:bg-white/6'

// ─── Component ────────────────────────────────────────────────────────────────

export function FlagActions({ flagId, wordId, initialData }: FlagActionsProps) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setEditing(false)
      // Note: no router.refresh() after save — only after resolve/dismiss
    } catch (err) {
  ...
  async function handleAction(action: 'resolve' | 'dismiss') {
    setActionStatus(action === 'resolve' ? 'resolving' : 'dismissing')
    setActionError('')

    try {
      const res = await fetch(`/api/admin/flags/${flagId}/${action}`, { method: 'POST' })
      const data = (await res.json()) as { success: boolean; error?: string }
      ...
      setActionStatus(action === 'resolve' ? 'resolved' : 'dismissed')
      router.refresh()
    } catch (err) {
  ...
  const busy = actionStatus === 'resolving' || actionStatus === 'dismissing'
    ...
    <div className="w-full space-y-4">
      ...
      {editing && (
        <div className="pt-4 border-t border-white/6 space-y-5">
          ...
          <div className="space-y-1.5">
            <label className="block font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground/80">
              Example sentence <span className="ml-1.5 normal-case tracking-normal font-body text-[10px]">(optional)</span>
            </label>
            <textarea
              value={example}
          ...
          <div className="space-y-1.5">
            <label className="block font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground/80">
              Etymology / origin <span className="ml-1.5 normal-case tracking-normal font-body text-[10px]">(optional)</span>
            </label>
            <input
              type="text"
              value={origin}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 h-9 px-4 bg-[#c8f135] text-black font-display font-bold text-xs uppercase tracking-wider rounded-lg hover:bg-[#d4f545] active:scale-95 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              onClick={cancelEdit}
              disabled={saving}
              className="inline-flex items-center gap-1.5 h-9 px-4 border border-white/10 text-muted-foreground/80 font-mono text-xs uppercase tracking-wider rounded-lg hover:border-white/20 active:scale-95 transition-all duration-150 disabled:opacity-50"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="flex flex-col items-start gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 h-8 px-3.5 bg-[#c8f135] text-black font-display font-bold text-xs uppercase tracking-wider rounded-lg hover:bg-[#d4f545] active:scale-95 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {actionStatus === 'resolving' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
            Resolve
          </button>
          ...
          <button
            ...
            disabled={busy}
            className="inline-flex items-center gap-1.5 h-8 px-3.5 bg-white/5 border border-white/10 text-muted-foreground/80 font-display font-bold text-xs uppercase tracking-wider rounded-lg hover:border-[#c8f135]/30 hover:text-[#c8f135] hover:bg-[#c8f135]/5 active:scale-95 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            disabled={busy || editing}
            className="inline-flex items-center gap-1.5 h-8 px-3.5 bg-white/5 border border-white/10 text-muted-foreground/80 font-display font-bold text-xs uppercase tracking-wider rounded-lg hover:border-red-500/30 hover:text-red-400/80 hover:bg-red-500/5 active:scale-95 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {actionStatus === 'dismissing' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <X className="w-3.5 h-3.5" />}
            Dismiss
          </button>
